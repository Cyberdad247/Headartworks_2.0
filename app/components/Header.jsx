...
export function Header({header, isLoggedIn, cart, publicStoreDomain}) {
  ...
  // Update header visibility and size based on video state and scroll position
  useEffect(() => {
    // Show header only after video has ended
    if (videoEnded) {
      // Calculate logo size based on scroll position (between 1 and 0.6)
      const newLogoSize = Math.max(0.6, 1 - (scrollPosition / 400));
      setLogoSize(newLogoSize);
      
      // Add shrink class when scrolled down
      if (scrollPosition > 30) {
        setHeaderClass('header visible shrink');
      } else {
  ...
  }, [videoEnded, scrollPosition]);
  
  return (
    <header className={headerClass}>
      <div className="header-logo-container" style={{transform: `scale(${logoSize})`}}>
        <NavLink prefetch="intent" to="/" style={activeLinkStyle} end>
          {shop.brand?.logo?.image ? (
            ...
            <strong className="header-logo-text">{shop.name}</strong>
          )}
        </NavLink>
      </div>
      <HeaderMenu
